...
@interface SFTestModelToJSONModel : NSObject
...
@implementation SFTestModelToJSONModel
...
@interface SFTestKeyPathModelToJSONModel : NSObject
...
@implementation SFTestKeyPathModelToJSONModel
+ (NSDictionary *)sf_modelCustomPropertyMapper {
    return @{
             @"a" : @"ext.a",
             @"b" : @"ext.b",
...
@interface SFTestModelToJSON : XCTestCase

@end

@implementation SFTestModelToJSON


- (void)testToJSON {
    SFTestModelToJSONModel *model = [SFTestModelToJSONModel new];
    model.intValue = 1;
    model.longValue = 2;
    ...
    NSDictionary *jsonObject = [model sf_modelToJSONObject];
    ...
    NSString *jsonString = [model sf_modelToJSONString];
    XCTAssert([[SFTestHelper jsonObjectFromString:jsonString] isKindOfClass:[NSDictionary class]]);
    
    NSData *jsonData = [model sf_modelToJSONData];
    XCTAssert([[SFTestHelper jsonObjectFromData:jsonData] isKindOfClass:[NSDictionary class]]);
    
    model = [SFTestModelToJSONModel sf_modelWithJSON:jsonData];
    XCTAssert(model.intValue == 1);
}

- (void)testKeyPath {
    SFTestKeyPathModelToJSONModel *model = [SFTestKeyPathModelToJSONModel new];
    model.a = @"a";
    model.b = @"b";
    model.c = @"c";
    model.d = @"d";
    ...
    model.f = @{@"g" : @""};
    dic = [model sf_modelToJSONObject];
    XCTAssert([dic[@"f"][@"g"] isEqualToString:@""]);
}
...
- (void)testDate {
    ...
    NSString *jsonString = [model sf_modelToJSONString];
    SFTestModelToJSONModel *newModel = [SFTestModelToJSONModel sf_modelWithJSON:jsonString];
